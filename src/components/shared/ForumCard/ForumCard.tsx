  ...
  return (
    ...
    >
      <h3
        ...
        data-testid={`forum-card-title-${forum.id}`}
      >
        {forum.title}
      </h3>
      <p
        className="text-gray-300 mb-4 line-clamp-3"
        data-testid={`forum-card-content-${forum.id}`}
      >
        {contentPreview}
      </p>
      <div className="flex items-center justify-between text-sm text-gray-400">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            aria-label={
              forum.liked_by_current_user ? "Unlike forum" : "Like forum"
            }
          ...
          >
            {forum.liked_by_current_user ? (
              <HeartIconFilled
                ...
                data-testid={`forum-card-like-icon-filled-${forum.id}`}
              />
            ) : (
              <HeartIcon
                className="h-5 w-5 text-blue-400 transition-colors"
                data-testid={`forum-card-like-icon-outline-${forum.id}`}
              />
            )}
