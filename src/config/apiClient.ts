...
apiClient.interceptors.request.use(
  (config: CustomAxiosRequestConfig) => {
    ...
    if (!requestThrottler.canMakeRequest(endpoint)) {
      ...
      throw new Error(
        `Request throttled. Please wait ${Math.ceil(timeUntilNext / 1000)} seconds.`,
      );
    }
    ...
    if (!apiCallLimiter.canMakeCall(endpoint)) {
      ...
      throw new Error(
        `API rate limit exceeded. Remaining: ${remaining.minute} per minute, ${remaining.hour} per hour.`,
      );
    }
    ...
    if (!behaviorDetector.recordAction(`api_call:${endpoint}`, userId)) {
      throw new Error(
        "Suspicious behavior detected. Please slow down your requests.",
      );
    }
