...
export default function CreateForumPage() {
  ...
  return (
    <>
      <SEOHead
        title="Create Forum - TT Cyclopedia"
        description="Start a new discussion in the TT Cyclopedia community. Create a forum topic to engage with other table tennis enthusiasts."
        noindex
      />
      <div
        className="max-w-2xl mx-auto px-4 py-8"
        data-testid="create-forum-page"
      >
        <h1
          className="text-2xl font-bold mb-6 text-white"
          data-testid="create-forum-title"
        >
          Create New Forum
        </h1>
        <form
          onSubmit={handleSubmit}
          className="space-y-6 bg-slate-900/80 p-6 rounded-lg shadow-lg border border-white/10"
        ...
        >
          ...
          {error && (
            <div
              ...
              data-testid="forum-error-message"
            >
              {error}
            </div>
          )}
          <div className="flex justify-end gap-2">
            <button
              type="button"
              className="px-4 py-2 rounded-lg bg-white/5 text-gray-300 hover:bg-white/10 border border-white/10 transition-colors"
