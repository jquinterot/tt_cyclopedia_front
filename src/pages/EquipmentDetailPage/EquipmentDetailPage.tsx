...
export default function EquipmentDetailPage() {
  ...
  const { posts: relatedPosts, isLoading: postsLoading } = usePostsByEquipment(
    id || "",
  );
    ...
    <>
      <SEOHead
        ...
        description={
          ...
          `${equipment.name} by ${equipment.brand} - Table tennis ${equipment.category}`
        }
            ...
            {equipment.avg_rating && (
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-gray-400 text-sm">
                  ({equipment.review_count} reviews)
                </span>
              </div>
            )}
        ...
        <div className="mt-8 grid md:grid-cols-2 gap-8">
          ...
          {isBlade && equipment.blade_specs && (
            <div className="bg-white/5 border border-white/10 rounded-lg p-6">
              ...
              <SpecBar
                label="Stiffness"
                value={equipment.blade_specs.stiffness}
              />
              <SpecBar
                label="Hardness"
                value={equipment.blade_specs.hardness}
              />
              ...
              {equipment.blade_specs.weight_min &&
                equipment.blade_specs.weight_max && (
                  <div className="flex justify-between text-sm py-2 border-t border-white/10">
                    <span className="text-gray-300">Weight</span>
                    <span className="text-white">
                      {equipment.blade_specs.weight_min}-
                      {equipment.blade_specs.weight_max}g
                    </span>
                  </div>
                )}
              {equipment.blade_specs.thickness && (
                <div className="flex justify-between text-sm py-2 border-t border-white/10">
                  ...
                  <span className="text-white">
                    {equipment.blade_specs.thickness}mm
                  </span>
                </div>
              )}
          ...
          {isRubber && equipment.rubber_specs && (
            <div className="bg-white/5 border border-white/10 rounded-lg p-6">
              ...
              <SpecBar
                label="Tackiness"
                value={equipment.rubber_specs.tackiness}
              />
              <SpecBar label="Grip" value={equipment.rubber_specs.grip} />
              <SpecBar
                label="Durability"
                value={equipment.rubber_specs.durability}
              />
              ...
              {equipment.rubber_specs.top_sheet && (
                <div className="flex justify-between text-sm py-2 border-t border-white/10">
                  ...
                  <span className="text-white capitalize">
                    {equipment.rubber_specs.top_sheet.replace("_", " ")}
                  </span>
                </div>
              )}
